...
// - REINDEX SIX   -> custom single-tag stub index
// - REINDEX SCX   -> custom compound stub index
...
static void print_reindex_help()
{
    std::cout
        ...
        << "      Rebuild CDX/LMDB backing store.\n"
        << "\n"
        << "  REINDEX SIX [<tagfile>]\n"
        << "      Custom single-tag stub index.\n"
        << "\n"
        << "  REINDEX SCX [<tagfile>]\n"
        << "      Custom compound stub index.\n"
        << "\n"
        << "  REINDEX ALL\n"
...
static void run_reindex_six(DbArea&, const std::string&)
{
    std::cout << "REINDEX SIX: Custom stub (not implemented yet)\n";
}

static void run_reindex_scx(DbArea&, const std::string&)
{
    std::cout << "REINDEX SCX: Custom stub (not implemented yet)\n";
}
...
void cmd_REINDEX(DbArea& A, std::istringstream& args) {
    ...
    if (argRest.empty()) {
        ...
        if (def == ReindexFamily::CDX) {
            std::cout << "REINDEX default -> CDX (v64-like table)\n";
