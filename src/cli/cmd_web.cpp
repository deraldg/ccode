...
static bool save_bytes(const std::string& path, const std::vector<char>& bytes, std::string& err)
{
...
void cmd_WEB(DbArea&, std::istringstream& S)
{
    ...
    if (sub == "GET" || sub == "HEAD") {
        ...
        std::cout << "\n";
#else
        std::cout << "WEB: not implemented on this platform\n";
#endif
        return;
    }
    ...
    if (sub == "FETCH") {
        ...
        for (char& c : toKw) c = static_cast<char>(std::toupper(static_cast<unsigned char>(c)));
        if (toKw != "TO") {
            std::cout << "WEB FETCH: expected TO <file>\n";
            return;
        }
